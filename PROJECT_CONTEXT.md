...
**最終更新**: 2026-04-27
**ステータス**: 本番稼働中（クライアント実機テスト中）
**バージョン**: v20260427010825
...
2. **クイック金額（+500等）削除済** → 復活させない（テンキーで足りる）
3. **iPadネイティブ数字キーボード呼び出し禁止** → アプリ内テンキー固守
4. **アラビア数字でのクイック表示** → 漢数字（五百/千等）は不可
