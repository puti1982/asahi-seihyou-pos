...
## 17. 引渡し前クリーンアップ手順（v18時点）

引渡し時に「真っさら状態」にするための実機操作手順。

### 17.1 必須操作（端末の店主が実行）

1. アプリ右上「設定」をタップ
2. 「データ管理」セクションへスクロール
3. 赤文字「**全ての販売履歴を消去**」をタップ
4. ブラウザ確認ダイアログで「OK」
5. カスタムダイアログ表示 → **3秒クールダウン待機** → 「削除」ボタン押下
6. トースト「販売履歴を全消去しました」が出れば完了
...
|---|---|
| 販売履歴（全取引） | ✓ 削除 |
| サンプル取引フラグ（SEEDED_KEY） | ✓ 起動時に自動クリア |
| 品書き 33味 | 保持 |
| トッピング3種（ミルク/スプーン/特製カップ） | 保持 |
| スリープ防止設定 | 保持 |
| カートドラフト（30分expiry） | 営業終了時に空ならば自動的に空のまま |

### 17.3 引渡し前確認チェックリスト

- [ ] 売上帳タップ → 「0 取引 / 0円」表示
- [ ] カート空（御注文欄に「氷」絵文字+空メッセージ表示）
- [ ] 品書き33味すべて表示（最終行: 巨峰＆ベリー）
- [ ] 設定画面の品書き一覧で33品（順次番号付き）
- [ ] スリープ防止トグル: 店主の希望に応じて ON/OFF
- [ ] PWAホーム画面アイコン: 暖簾デザイン（紺地+白氷）

### 17.4 緊急時の完全リセット

クリーンアップ後にトラブルがあれば：
1. ホーム画面アイコン長押し → 削除
2. Chrome「設定 → プライバシー → 閲覧履歴データの削除」（過去24時間）
3. ChromeでURL再アクセス → 「ホーム画面に追加」
4. 起動時に DEFAULT_FLAVORS / DEFAULT_TOPPINGS が自動投入される
